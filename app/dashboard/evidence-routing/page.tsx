  ...
  return (
    <main className="product-page routing-page">
      ...
      <div className="routing-shell">
        <section className="dashboard-summary"><article><span>Pending requests</span><strong>{pendingInbound}</strong><small>Waiting for this organization</small></article><article><span>Active grants</span><strong>{activeGrants}</strong><small>Controller-defined recipient scopes</small></article><article><span>Delivered TECRIDs</span><strong>{deliveries.length}</strong><small>Frozen recipient views</small></article><article><span>Lab routes</span><strong>{authorizations.filter((item) => item.status === "active").length}</strong><small>Revocable delivery tokens</small></article></section>
        ...
        <EvidenceRoutingClient organization={{ id: organizationId, name: data.membership.organization.name, code: data.membership.organization.issuerCode, type: data.membership.organization.organizationType }} requests={requests} grants={grants} deliveries={deliveries} authorizations={authorizations} />
      </div>
      <ProductFooter />
    </main>
  );
