...
export function ProfileSettingsForm({ organization, profile }: {
  ...
  return (
    <form className="profile-settings-form" id="public-profile" onSubmit={save}>
      <div className="settings-section-heading"><div><p className="section-kicker">Public participant profile</p><h2>Choose what the network may show.</h2></div><span>{profile?.isPublic ? "Public" : "Private"}</span></div>
      ...
      <div className="settings-form-actions"><button className="button-dark" type="submit" disabled={pending}>{pending ? "Saving…" : "Save profile settings →"}</button>{profile?.isPublic ? <Link href={profileUrl}>View public profile ↗</Link> : <Link href="/participants">View participant directory ↗</Link>}</div>
